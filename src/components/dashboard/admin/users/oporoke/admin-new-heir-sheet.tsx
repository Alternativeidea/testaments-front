...
import { PhoneAreaCodesCombobox } from '@/components/dashboard/ui/phone-area-codes-combobox'
...
import { createHeir, deleteHeir, updateHeir } from '@/lib/services/admin/users/heirs'
...
interface AdminNewHeirStoreProps {
    isOpen: boolean
...
export const useAdminNewHeirState = create<AdminNewHeirStoreProps>()((set) => ({
    isOpen: false,
 ...
 * Main
 * ------------------------------------------------------------------------------------------- */
interface AdminNewHeirSheetProps{
    heirs: HeirProps[]
    isShowTrigger?: boolean
}

export function AdminNewHeirSheet({ heirs, isShowTrigger }: AdminNewHeirSheetProps) {
    const isOpen = useAdminNewHeirState(s => s.isOpen)
    const setIsOpen = useAdminNewHeirState(s => s.setIsOpen)
    const currentStep = useAdminNewHeirState(s => s.currentStep)
    const setCurrentStep = useAdminNewHeirState(s => s.setCurrentStep)
    const setCurrentHeir = useAdminNewHeirState(s => s.setCurrentHeir)
    const setCurrentActionSuccess = useAdminNewHeirState(s => s.setCurrentActionSuccess)
...
function HeirResumeStep({ className }: StepComponentProps) {
    ...
    const setCurrentStep = useAdminNewHeirState(s => s.setCurrentStep)
    const currentHeir = useAdminNewHeirState(s => s.currentHeir)
    const setCurrentActionSuccess = useAdminNewHeirState(s => s.setCurrentActionSuccess)
    ...
    async function handleConfirm() {
        try {
            setIsLoading(true)
            await createHeir(userId, currentHeir)
            setCurrentActionSuccess('dodan')
            setCurrentStep(6)
        } catch (error) {
            if (error instanceof Error) {
...
function HeirEditResumeStep({ className }: StepComponentProps) {
    ...
    async function handleConfirm() {
        try {
            setIsLoading(true)
            await updateHeir(userId, currentHeir.id ?? 0, currentHeir)
...
function HeirDeleteConfirmationStep({ className }: StepComponentProps) {
    ...
    async function handleConfirm() {
        try {
            setIsLoading(true)
            await deleteHeir(userId, currentHeir.id ?? 0)
...
function HeirSuccessStep({ className }: StepComponentProps) {
    const currentActionSuccess = useAdminNewHeirState(s => s.currentActionSuccess)
