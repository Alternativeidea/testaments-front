...
export default function ReceiveTestamentForm({ email } : {email : string}) {
    ...
    return (
        <Card className="flex flex-col justify-start w-full h-full min-h-full border-none overflow-x-scroll overflow-y-hidden no-scrollbar">
            ...
            <CardContent className='flex flex-col gap-y-4 px-4 h-full py-0 mt-[100px]'>
                ...
                <div className='group flex transition-colors items-center w-full justify-between relative border border-black hover:bg-primary-dark-gray hover:text-primary-white'>
                    <Button onClick={() => copyEmail(email)} variant='light' className='w-[90%] justify-between relative overflow-x-scroll overflow-y-hidden border-none group-hover:bg-primary-dark-gray group-hover:text-primary-white'>
                        {email}
                    </Button>
                    <Icons.Copy
