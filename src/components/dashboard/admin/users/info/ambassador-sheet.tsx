...
import { convertToAmbassadorId } from '@/lib/services/ambassadors'
import { cn } from '@/lib/utils/style'
import { useEffect, useState } from 'react'
import { toast } from 'sonner'

interface AmbassadorSheetProps {
    userId: number
}

export function AmbassadorSheet({ userId }: AmbassadorSheetProps) {
    ...
    async function handleIsAmbassador() {
        try {
            setIsLoadingConvert(true)
            await convertToAmbassadorId(userId)
            setIsAmbassador(a => !a)
            setIsUpdated(true)
        ...
        <Sheet onOpenChange={handleOpenChange}>
            <SheetTrigger asChild>
                {isLoading
                    ...
                    : <Switch checked={isAmbassador} className={cn(isAmbassador ? 'bg-primary' : 'bg-primary-light-gray')}/>
                }
            </SheetTrigger>
                ...
                >
                    {!isAmbassador && !isUpdated && (
                        <div className={cn('flex flex-auto flex-col')}>
                            <SheetHeader className='flex flex-auto justify-center items-center w-full px-7 pt-10 pb-8 space-y-0 relative z-[100]'>
                                ...
                                <span className="text-h5 font-baskerville text-center !mb-8">Ste prepričani da želite da postane uporabnik Svetovalec?</span>
                            </SheetHeader>
                    ...
                    {isAmbassador && !isUpdated && (
                        <div className={cn('flex flex-auto flex-col')}>
                            <SheetHeader className='flex flex-auto justify-center items-center w-full px-7 pt-10 pb-8 space-y-0 relative z-[100]'>
                                <Ilustrations.List className='w-full max-w-44 lg:max-w-72' />
                                <span className="text-h5 font-baskerville text-center !mb-8">Ste prepričani, da želite uporabniku izključiti Svetovalec funkcijo?</span>
                            </SheetHeader>
                            <SheetFooter className='flex items-center justify-center w-full bg-primary-white px-6 py-4 mt-auto'>
                                <Button
                                    disabled={isLoadingConvert}
